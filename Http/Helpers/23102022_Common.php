...
class Common {
    ...
    public function mesCorte($fechaInicio,$fechaFin){

        $fechaInicio1=strtotime($fechaInicio);
        
        $fechaFin=strtotime($fechaFin);
        $fechaMesCorte="";
        $n=1;
        for($i=$fechaInicio1; $i<=$fechaFin; $i+=86400){
        
            /*Obtener la fecha de corte de cada mes*/
            $date1=date_create($fechaInicio);
            $date2=date_create(date("Y-m-d", $i));
            $diff=date_diff($date1,$date2);
            $meses = $diff->format("%m");
            
            if($meses>=$n){
                $fechaMesCorte=date("Y-m-d", $i);
                $n++;
            } 
            
        }
        
        return $fechaMesCorte;
    }

    public function semanaCorte($fechaInicio,$fechaFin){
        $fechaInicio1=strtotime($fechaInicio);
        $fechaFin=strtotime($fechaFin);
        $fechaSemanaCorte="";
        $n=7;
        for($i=$fechaInicio1; $i<=$fechaFin; $i+=86400){
            /*Obtener la fecha de corte de la ultima semana*/
            $n++;
            $date11=date_create($fechaInicio);
            $date22=date_create(date("Y-m-d", $i));
            $diff_sem=date_diff($date11,$date22);
            $dias = $diff_sem->format("%a");
            
            if(($dias%7)==0 && $dias>0){
                $fechaSemanaCorte=date("Y-m-d", $i);
                $date11=date_create(date("Y-m-d", $i));
            } 
            
        }
        
        return $fechaSemanaCorte;
    }
    
    public function getPrice($propertyId, $checkIn, $checkOut, $guestCount, $startTime, $endTime, $force = false)
    {
        ...
        /*
            Parametros a hacer el calculo respecto al precio
        ...
        */
        ...
        $var_tipo = "";
        $mesCorteFecha="";
        $mesCorteFecha = $this->mesCorte($checkIn,$checkOut);
        $semanaCorteFecha="";

        if(!empty($mesCorteFecha)){
            $semanaCorteFecha = $this->semanaCorte($mesCorteFecha,$checkOut);
        }else{
            $semanaCorteFecha = $this->semanaCorte($checkIn,$checkOut);
        }
        ...
        $result['property_default'] = [
            'price' => $property->property_price->price_day,
            'currency_code' => $property->property_price->currency_code_day,
            'symbol' => Currency::getAll()->firstWhere('code', $property->property_price->currency_code_day)->symbol,
            'rate' => Currency::getAll()->firstWhere('code', $property->property_price->currency_code_day)->rate,
            'local_to_propertyRate' => $this->currency_rate($property->property_price->currency_code_day, $this->getCurrentCurrencyCode()),
        ];
        
        $differentPriceDatePrice = $force ? $propertyDates->pluck("price_day", 'date')->toArray()
            : $propertyDates->where('status', 'Available')->pluck("price_day", 'date')->toArray();
        $differentPrice = 0;
        $weekendPrice = 0;
        $nightPrice = 0;
        $monthPrice = 0;
        ...
        foreach ($dates as $date) {
            ...
            $propertyDates2 = PropertyDates::Select('price','price_day','price_week','price_month')->where(['property_id' => $propertyId])->where('date', $date)->first();

            $horas=1;
            if($startTime=="00:00:00" && $endTime=="23:59:00"){
                if(!empty($mesCorteFecha) && $date < $mesCorteFecha){
                    $var_tipo = "price_month";
                }elseif(!empty($semanaCorteFecha) && $date < $semanaCorteFecha ){
                    $var_tipo = "price_week";
                }else{
                    $var_tipo = "price_day"; 
                }
                ...
                $var_tipo = "price";
                $hour1=date_create($startTime);
                $hour2=date_create($endTime);
                $diff=date_diff($hour1,$hour2);
                $horas = $diff->format("%h");

            }

            if (in_array($date, array_keys($differentPriceDatePrice))) {
                $differentPrice += $this->convert_currency($property->property_price->currency_code, '', (($propertyDates2->$var_tipo)*$horas));
        ...
        foreach ($dates as $key => $value) {
            ...
            $horas=1;
            if($startTime=="00:00:00" && $endTime=="23:59:00"){
                if(!empty($mesCorteFecha) && $value < $mesCorteFecha){
                    $var_tipo = "price_month";
                }elseif(!empty($semanaCorteFecha) && $value < $semanaCorteFecha ){
                    $var_tipo = "price_week";
                }else{
                    $var_tipo = "price_day"; 
                }
            ...
            if (in_array($value, array_keys($differentPriceDatePrice))) {
                ...
                $allDate[$key]['price'] = moneyFormat($symbol, numberFormat($this->convert_currency($property->property_price->currency_code_, '', (($propertyDates2->$var_tipo)*$horas)), $decimals));
                $allDate[$key]['original_price'] = $this->convert_currency($property->property_price->currency_code, '', $propertyDates2->$var_tipo);
                ...
                $allDate[$key]['price'] = moneyFormat($symbol, numberFormat($property->property_price->$var_tipo, $decimals));
                $allDate[$key]['original_price'] = $property->property_price->$var_tipo;
