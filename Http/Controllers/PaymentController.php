...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            Session::put('payment_start_time', $request->start_time);
            Session::put('payment_end_time', $request->end_time);
            ...
            $start_time       = Session::get('payment_start_time');
            $end_time         = Session::get('payment_end_time');
        ...
        $date1 = date('Y-m-d', strtotime(setDateForDb($checkin)));
        $enddate = date('Y-m-d', strtotime(setDateForDb($checkout)));
        ...
        $data['price_list']    = json_decode($this->helper->getPrice($data['property_id'], $data['checkin'], $data['checkout'], $data['number_of_guests'], $data['start_time'], $data['end_time']));
                                                            //getPrice($propertyId, $checkIn, $checkOut, $guestCount, $startTime, $endTime, $force = false)
    ...
    public function createBooking(Request $request)
    {
        ...
        $price_list         = json_decode($this->helper->getPrice($request->property_id, $request->checkin, $request->checkout, $request->number_of_guests,$request->start_time,$request->end_time));
