...
class PropertyPrice extends Model
{
    ...
    protected $appends = [ 'original_cleaning_fee', 'original_guest_fee', 'original_price','original_price_day','original_price_week','original_price_month', 'original_weekend_price', 'original_security_fee', 'default_code', 'default_symbol'];
    ...
    public function getOriginalPriceDayAttribute()
    {
        return $this->attributes['price_day'];
    }

    public function getOriginalPriceWeekAttribute()
    {
        return $this->attributes['price_week'];
    }

    public function getOriginalPriceMonthAttribute()
    {
        return $this->attributes['price_month'];
    }
    ...
    //Original Property Price_day
    public function original_price_day($date)
    {      
        $result = PropertyDates::getTempDates()
            ->where('property_id', $this->attributes['property_id'])
            ->where('date', $date);
        ...
        try {
            return $result->first()->price_day;
        } catch (\Exception $e) {
            return $this->attributes['price_day'];
        }
    }

    //Original Property Price_week
    public function original_price_week($date)
    {      
        $result = PropertyDates::getTempDates()
            ->where('property_id', $this->attributes['property_id'])
            ->where('date', $date);
        ...
        try {
            return $result->first()->price_week;
        } catch (\Exception $e) {
            return $this->attributes['price_week'];
        }
    }

    //Original Property Price_month
    public function original_price_month($date)
    {      
        $result = PropertyDates::getTempDates()
            ->where('property_id', $this->attributes['property_id'])
            ->where('date', $date);
        ...
        try {
            return $result->first()->price_month;
        } catch (\Exception $e) {
            return $this->attributes['price_month'];
        }
    }
    ...
    public function starttime($date)
    {
        $result = PropertyDates::getTempDates()
            ...
            ->where('date', $date);
        try {
            return $result->first()->start_time;
        } catch (\Exception $e) {
            return '00:00:00';
        }
    }
